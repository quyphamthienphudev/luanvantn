    ...
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow p-6">
        ...
        <form action="/hcns/payrolls/update/{{ $payroll->id }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select name="month" class="w-full border rounded px-3 py-2">
                        @for($i = 1; $i <= 12; $i++) <option value="{{ $i }}" {{ $payroll->month == $i ? 'selected' : ''
                            }}>Tháng {{ $i }}</option>
                        @endfor
                    </select>
                </div>
                <div>
                    ...
                    <select name="year" class="w-full border rounded px-3 py-2">
                        @for($i = 2001; $i <= 2099; $i++) <option value="{{ $i }}" {{ $payroll->year == $i ?
                            'selected' : '' }}>Năm {{ $i }}</option>
                        @endfor
                    </select>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" value="{{ ($payroll->base_salary + $payroll->bonus - $payroll->deduction) * 0.1 }}" class="w-full border p-2 rounded bg-gray-100" readonly>
            </div>

                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center align-items-center gap-2 action-group ">
                                @if($leave->status == 'pending')
                                    ...
                                    <form action="/qlcl/leave/reject/{{ $leave->id }}" method="POST" class="m-0 p-0" onsubmit="return confirm('Từ chối đơn nghỉ phép này ?')">
                                        @csrf
                                        <button class="btn btn-outline-danger btn-sm btn-action" title="Từ chối"><i class="fas fa-ban"></i></button>
                                    </form>

                                    <a href="/qlcl/leave/edit/{{ $leave->id }}" class="btn btn-info btn-sm btn-action text-white" title="Chỉnh sửa">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                @endif

                                <form action="/qlcl/leave/delete/{{ $leave->id }}" method="POST" class="m-0 p-0" onsubmit="return confirm('Bạn có muốn xóa đơn xin nghỉ phép này?')">
                                    @csrf
                                    @method('DELETE')

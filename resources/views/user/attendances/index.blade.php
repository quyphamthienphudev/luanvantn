...
<body>
    <div @class(['max-w-md', 'mx-auto', 'mt-4', 'px-4'])>
        ...
        <form action="/attendances/checkin" method="post">
            @csrf
            <button type="submit" @class(['w-full', 'bg-blue-600', 'text-white', 'py-4', 'rounded-lg', 'text-lg', 'font-semibold', 'hover:bg-blue-700']) title="Xác nhận chấm công">Xác nhận chấm công</button>
        </form>
        @else
        <span @class(['block', 'w-full', 'text-center', 'bg-green-600', 'text-white', 'py-4', 'rounded-lg', 'text-lg', 'font-semibold'])>Đã xác nhận</span>
        @endif
        <br><br>
        <div @class(['flex', 'justify-between', 'items-center', 'mb-6'])>
        ...
        <span @class(['block', 'w-full', 'text-center', 'bg-green-600', 'text-white', 'py-4', 'rounded-lg', 'text-lg', 'font-semibold'])>Đã xác nhận</span>
        @else
        <span @class(['text-red-600'])>Vui lòng chấm công vào làm trước</span>
        @endif
    </div>
</body>

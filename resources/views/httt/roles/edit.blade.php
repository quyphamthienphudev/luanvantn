...
<body>
    ...
    <h1 @class(['text-2xl', 'font-bold', 'mb-6'])>Cập nhật quyền truy cập</h1>
    <form action="/httt/roles/update/{{ $roles->id }}" method="post" @class(['bg-white', 'p-6', 'rounded', 'shadow', 'w-1/2'])>
        @csrf
        <div @class(['mb-4'])>
            <label>Tên quyền truy cập</label>
            <input type="text" value="{{ $roles->name }}" @class(['w-full', 'border', 'p-2', 'rounded', 'bg-gray-100']) readonly>
        </div>
        <div @class(['mb-4'])>
            <label>Mô tả</label>
            <input type="text" name="description" value="{{ old('description', $roles->description) }}" @class(['w-full', 'border', 'p-2', 'rounded']) placeholder="Mô tả">
            @error('description')

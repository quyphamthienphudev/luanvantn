...
<body>
    <div @class(['flex', 'space-x-2'])>
        <a href="/httt/accounts/create" @class(['bg-blue-600', 'text-white', 'px-4', 'py-2', 'rounded', 'hover:bg-blue-700']) title="Thêm tài khoản">Thêm tài khoản</a>
        <form action="/httt/accounts/export" method="post">
            @csrf
            <button @class(['bg-yellow-600', 'text-white', 'px-4', 'py-2', 'rounded', 'hover:bg-yellow-700']) title="Xuất Excel">Xuất Excel</button>
        </form>
    </div>
    <form action="/httt/accounts" method="get" @class(['mt-4'])>
        @csrf
        Tìm kiếm: <input type="text" name="search" value="{{ $search }}" @class(['border', 'p-2']) placeholder="Tìm theo họ tên hoặc email" style="width:250px;">
            ...
            <tbody>
                ...
                <tr @class(['border-b'])>
                    ...
                    <td @class(['flex', 'space-x-2', 'p-5'])>
                        <a href="/httt/accounts/edit/{{ $u->id }}" @class(['bg-yellow-500', 'text-white', 'px-3', 'py-1', 'rounded']) title="Sửa">Sửa</a>
                        <form action="/httt/accounts/delete/{{ $u->id }}" method="post">
                            @csrf 
                            <button @class(['bg-red-600', 'text-white', 'px-3', 'py-1', 'rounded']) onclick="return confirm('Bạn có muốn xoá tài khoản này ?')" title="Xoá">Xoá</button>
                        </form>
                    </td>
                </tr>
                @empty

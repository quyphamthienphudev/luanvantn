...
<body>
    ...
    <form method="POST" action="/hcns/disciplines/update/{{ $disciplines->id }}" class="bg-white p-6 w-1/2">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" value="{{ $disciplines->title }}" class="w-full border p-2 rounded" placeholder="Nội dung kỷ luật">
        </div>
        <div class="mb-4">
            <label>Số tiền</label>
            <input type="text" name="amount" value="{{ $disciplines->amount }}" class="w-full border p-2 rounded" placeholder="Số tiền">
        </div>
        <div class="mb-4">
            <label>Ngày ra quyết định</label>
            <input type="date" name="decision_date" value="{{ $disciplines->decision_date }}" class="w-full border p-2 rounded">
        </div>
